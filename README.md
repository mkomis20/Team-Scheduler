...
- **Individual Restore**: Upload CSV files to restore specific data types
  - Restore WFH Records from previous backups
  - Restore Annual Leave Records from previous backups
  - Restore Seminar Attendance Records from previous backups
  - Restore Leave Balance data from previous backups
  - Validates file format before importing
  - Shows record count after successful restoration
- **Bulk Restore**: Upload complete backup file for full system recovery
  - Restores all data types in a single operation
  - Imports from complete backup files generated by "Export All Data"
  - Perfect for system migration or disaster recovery
  - Shows progress and record counts during restoration
- **Safety Features**:
  - Warning message before overwriting existing data
  - File validation to ensure backup file integrity
  - Error messages if restoration fails
  - Automatic page refresh after successful restoration
  - Instructions for proper backup/restore workflow
